...
object Global extends WithFilters(CORSFilter()) with GlobalSettings {
  ...
  override def onStart(app: Application) {
    ...
    if (Schema.hasGoldData(app)) {
      println("Database is not empty...using existing db")
    } else {
      println("Database doesn't have gold data. Please set it up first!")
      println("Proceeding with automatic gold data setup...")
      // populate schema
      println("Popluating schema...")
      Schema.populateSchema(app)
      // load sample data
      println("Popluating sample data...")      
